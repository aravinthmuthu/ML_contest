...
X_train, X_val, Y_train, Y_val = train_test_split(X, indices_to_one_hot(Y,29), test_size=0.2, random_state=42)


# input X: 28x28 grayscale images, the first dimension (None) will index the images in the mini-batch
X = tf.placeholder(tf.float32, [None,  2600])
...
W1 = tf.Variable(tf.truncated_normal([2600, L], stddev=0.1))  # 784 = 28 * 28
B1 = tf.Variable(tf.zeros([L]))
W2 = tf.Variable(tf.truncated_normal([L, O], stddev=0.1))
B2 = tf.Variable(tf.zeros([O]))
# W3 = tf.Variable(tf.truncated_normal([M, N], stddev=0.1))
# B3 = tf.Variable(tf.zeros([N]))
# W4 = tf.Variable(tf.truncated_normal([N, O], stddev=0.1))
# B4 = tf.Variable(tf.zeros([O]))
W5 = tf.Variable(tf.truncated_normal([O, 29], stddev=0.1))
B5 = tf.Variable(tf.zeros([29]))

# The model
XX = tf.reshape(X, [-1, 2600])
...
allweights = tf.concat([tf.reshape(W1, [-1]), tf.reshape(W2, [-1]), tf.reshape(W5, [-1])], 0)
allbiases  = tf.concat([tf.reshape(B1, [-1]), tf.reshape(B2, [-1]), tf.reshape(B5, [-1])], 0)
...
def training_step(i, update_test_data, update_train_data):
    ...
    if update_test_data:
        a, c = sess.run([accuracy, cross_entropy], {X:X_val, Y_:Y_val })
