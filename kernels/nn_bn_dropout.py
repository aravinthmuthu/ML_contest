# Added batchnorm layers to every hidden layer
...
Y1 = tf.nn.sigmoid(tf.contrib.layers.batch_norm(tf.matmul(XX, W1) + B1))
Y1_d = tf.nn.dropout(Y1, keep_prob)
Y2 = tf.nn.sigmoid(tf.contrib.layers.batch_norm(tf.matmul(Y1_d, W2) + B2))
Y2_d = tf.nn.dropout(Y2, keep_prob)
Y3 = tf.nn.sigmoid(tf.contrib.layers.batch_norm(tf.matmul(Y2_d, W3) + B3))
...
def training_step(i, update_test_data, update_train_data):
    ...
    if update_train_data:
        ...
        train_summ, a, c = sess.run([train_acc_summary, accuracy, cross_entropy], {X: batch_X, Y_: batch_Y, keep_prob: 0.5, phase_train: True})
    ...
    sess.run(train_step, {X: batch_X, Y_: batch_Y, keep_prob:0.5, phase_train: True})
